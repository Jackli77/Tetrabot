...
class chal(BaseCommand):
    ...
    async def handle(self, params, message, client):
        ...
        await channel.send(msg0)
        ad_id = int(''.join(filter(str.isdigit, message.author.mention)))
        adv_usr = await Client.fetch_user(client, ad_id)
        def check(reaction, user):
            return str(reaction.emoji) == '✅'
        try:
            reaction, adv_user = await client.wait_for('reaction_add', timeout=60.0, check=check)
        except asyncio.TimeoutError:
            await channel.send('Timeout')
        else:
            await channel.send('Challenge accepté')
